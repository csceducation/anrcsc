...
class AttendanceManagerV2:
    ...
    def put_lab_data(self, lab_no, date, sys, time, student, staff):
        ...
        if not doc:
            ...
            # Check if the time slot exists and update it
            result = self.db.lab_collection.update_one(
                ...
                {
                    "$addToSet": {
                        "data.$.students": student,  # Prevent duplicate students
                        "data.$.incharges": staff   # Prevent duplicate staff
                    }
                }
            )
            ...
            # If the time slot doesn't exist, add a new one
            new_time_slot = {
                "start": time['start'],
                "end": time['end'],
                "students": [student],
