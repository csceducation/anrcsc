...
def add_lab_data(request,lab_id,date):
    ...
    result = manager.put_lab_data(lab_id,date,sys,data['time'],data['student_id'],data['staff_id'])
    # debug_info(result)
    
    return JsonResponse({'message':result,"result":True,"name":stud[0].student_name})
